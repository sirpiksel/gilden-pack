# gilden-datapack interface load function #

scoreboard objectives add globalSettings trigger
scoreboard objectives add gs trigger

scoreboard objectives add playerSettings trigger
scoreboard objectives add ps trigger

scoreboard objectives add gamemode trigger
scoreboard objectives add gm trigger

scoreboard objectives add actionbar trigger
scoreboard objectives add a trigger

scoreboard objectives add spawn trigger
scoreboard objectives add s trigger

scoreboard objectives add clu trigger
scoreboard objectives add c trigger

scoreboard objectives add rinzler trigger
scoreboard objectives add r trigger